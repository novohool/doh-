...
func dohQueryARecord(domain string) (string, error) {
	...
	for _, ans := range result.Answer {
		if ans.Type == 1 { // A record
			if *verbose {
				log.Printf("[VERBOSE] DoH resolved %s to %s", domain, ans.Data)
			}
			return ans.Data, nil
		}
	}
...
func doSmartRequest(origReq *http.Request) (*http.Response, error) {
	...
	port := "443"
	ip, err := dohQueryARecord(origHost)
	if err != nil {
...
func printHelp() {
	fmt.Println(`gproxy - Advanced HTTP proxy and DNS resolution tool

Usage:
  gproxy -L http://:port [--doh URL] [-v]
  gproxy [curl-style options] <URL> [-v] [-I]
...
Examples:
  gproxy -L http://:8080 -v
  gproxy https://www.google.com -v
  gproxy -I https://example.com
`)
}
...
func main() {
	...
	args := flag.Args()
	if len(args) < 1 {
		log.Fatal("Usage: gproxy [options] URL")
	}
	requestURL = args[0]
